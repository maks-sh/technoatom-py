            ...
            $.ajax({
                ...
                success: function (rest_data) {
                    ...
                    if (/rest/i.test(window.location.pathname)) {
                        ...
                        $.ajax({
                            url: "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + rest_data.coords + "&key=AIzaSyAMciCzTf0qkhZ_irsqOgUUuLSBcQaNIqc",
                            type: 'GET',
            ...
            $('#signout').click(function(event) {
             ...
             return cookieValue;
                ...
                $.ajax({
                    ...
                    success: function () {
                        window.location.href = "http://127.0.0.1:8000/j-login/";
                    }
                });
            });
            $('#prev').click(function() {
    ...
    if (data.length == 0) {
        $('#books').text('There is no reservation')
            ...
            data.forEach(function(item, i, arr) {
                $('#books').html(book(item.user.phone_number, item.user.email, item.date, item.time.time, item.seats))
        })
    }

    function book(phone, email, date, time, seats){
        html = '<div class="book">' +
                        '<div class="book__icon-add"></div>' +
                        '<div class="book__text">' +
                            '<p class="book_field field__name bold"> Sergey Ivanov</p>' +
        ...
        html += '<p class="book_field field__date"><span class="bold">Reservation date: </span>' +
                ...
                '<div id="switch" class="switcher" onclick="confirmed(this)"><div id="toggle"></div></div></div></div>';
        return html;
    }
}